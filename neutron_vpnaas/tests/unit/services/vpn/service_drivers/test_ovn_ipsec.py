...
class TestOvnIPsecDriver(base.BaseTestCase):
    def setUp(self):
        ...
        self._fake_router = Munch({
            'id': FAKE_ROUTER_ID,
            'gw_port': Munch({'network_id': _uuid()})
        })
        ...
        self.svc_plugin._get_vpnservice.return_value = Munch({
            'router_id': FAKE_ROUTER_ID,
    ...
    def _test_ipsec_site_connection(self, old_peers, new_peers,
        ...
        self.svc_plugin.get_vpn_gw_by_router_id.return_value = Munch({
            'id': _uuid(),
            'router_id': FAKE_ROUTER_ID,
            'router': self._fake_router,
