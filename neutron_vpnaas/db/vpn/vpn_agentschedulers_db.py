...
# Copyright (c) 2020 SysEleven GmbH.
# All Rights Reserved.
#
...
class RouterVPNAgentBinding(model_base.BASEV2):
    ...
    router_id = sa.Column(sa.String(36),
                          ...
                          primary_key=True,
                          unique=True)
    vpn_agent_id = sa.Column(sa.String(36), primary_key=True)
...
class VPNAgentSchedulerDbMixin(
    ...
    def create_router_to_agent_binding(self, context, router_id, agent_id):
        ...
        try:
            ...
            LOG.debug('VPN service of router %(router_id)s has already been '
                      'scheduled to VPN agent %(agent_id)s.',
                      {'agent_id': agent_id,
                       'router_id': router_id})
            return False
    ...
    def add_router_to_vpn_agent(self, context, agent_id, router_id):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            ...
            if not self.check_agent_router_scheduling_needed(
                ...
                return
        try:
            self.create_router_to_agent_binding(
    ...
    def reschedule_router(self, context, router_id, candidates=None):
        ...
        with db_api.CONTEXT_WRITER.using(context):
            cur_agent_ids = [agent['id'] for agent in cur_agents]
            self._unschedule_router(context, router_id, cur_agent_ids)
