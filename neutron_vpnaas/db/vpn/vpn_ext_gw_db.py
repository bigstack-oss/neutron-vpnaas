...
#    (c) Copyright 2020 SysEleven GmbH
# All Rights Reserved.
#
...
class VPNGWInUse(n_exc.BadRequest):
    message = _(
...
class VPNExtGW(model_base.BASEV2, model_base.HasId, model_base.HasProject):
    ...
    transit_network = orm.relationship(models_v2.Network, lazy='joined')
    transit_subnet = orm.relationship(models_v2.Subnet, lazy='joined')
    router = orm.relationship(l3_models.Router, lazy='joined')
...
class VPNExtGWPlugin_db(object):
    ...
    def _get_gateway(self, context, gateway_id):
        try:
            return model_query.get_by_id(context, VPNExtGW, gateway_id)
        except exc.NoResultFound:
            return None

    def get_gateway(self, context, gateway_id):
        gateway_db = model_query.get_by_id(context, VPNExtGW, gateway_id)
        if gateway_db:
            return self._make_vpn_ext_gw_dict(gateway_db)

    def raise_if_gateway_is_used(self, context, gateway_db):
        ...
        if vpnservices:
            ...
            raise VPNGWInUse(gateway_id=gateway_db['id'], services=services)
    ...
    def update_gateway(self, context, gateway_id, gateway):
        info = gateway['gateway']
        with db_api.CONTEXT_WRITER.using(context):
            gateway_db = self._get_gateway(context, gateway_id)
